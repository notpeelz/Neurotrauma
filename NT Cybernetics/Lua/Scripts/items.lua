...
NTCyb.ItemMethods.fpgacircuit = function(item, usingCharacter, targetCharacter, limb)
  ...
  if not NTCyb.HF.LimbIsCyber(targetCharacter, limbtype) then
    return
  end
  if HF.GetAfflictionStrengthLimb(targetCharacter, limbtype, "ntc_damagedelectronics", 0) < 0.1 then
    return
  end
...
NTCyb.ItemMethods.steel = function(item, usingCharacter, targetCharacter, limb)
  ...
  if HF.GetAfflictionStrengthLimb(targetCharacter, limbtype, "ntc_materialloss", 0) < 0.1 then
    return
  end
...
NTCyb.ItemMethods.weldingtool = function(item, usingCharacter, targetCharacter, limb)
  ...
  if HF.GetAfflictionStrengthLimb(targetCharacter, limbtype, "ntc_bentmetal", 0) < 0.1 then
    return
  end
...
NTCyb.ItemMethods.cyberarm = function(item, usingCharacter, targetCharacter, limb)
  ...
  if NTCyb.HF.LimbIsCyber(targetCharacter, limbtype) then
    return
  end
  if not NT.LimbIsSurgicallyAmputated(targetCharacter, limbtype) then
    return
  end

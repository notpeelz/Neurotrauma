function NTCyb.UpdateHuman(character)
  ...
  local function updateLimb(character, limbtype)
    if not NTCyb.HF.LimbIsCyber(character, limbtype) then
      return
    end
    ...
    local damagedelectronics =
      HF.GetAfflictionStrengthLimb(character, limbtype, "ntc_damagedelectronics", 0)
    ...
    if loosescrews > 30 and velocity > 1 then
      ...
      HF.AddAfflictionLimb(
        ...
        "ntc_loosescrews",
        limbtype,
        HF.Clamp(velocity, 0, 5) / 50 * NT.Deltatime
      )
    end

...
function NTP.PillConfigFromItems(components, skill, descriptionOverride, user)
  ...
  for i, itemidentifier in ipairs(components) do
    ...
    if NTP.PillData.items[itemidentifier] ~= nil then
      s = math.random() < skill / (NTP.PillData.items[itemidentifier].skillrequirement or 30)
    end
    success[i] = s
  end
  ...
  for i, itemidentifier in ipairs(components) do
    if NTP.PillData.items[itemidentifier] ~= nil then
      ...
      if not success[i] then
        effects = NTP.PillData.items[itemidentifier].faileffects or effects
      end
  ...
  if activecombo ~= nil then
    if activecombo.coloroverride ~= nil then
      color =
        { activecombo.coloroverride[1], activecombo.coloroverride[2], activecombo.coloroverride[3] }
    end

...
function NTP.TryCraftPills(chemmaster, user, dontreporterrors)
  ...
  local config = NTP.PillConfigFromItems(
    ingredientArray,
    HF.GetSkillLevel(user, "medical"),
    descriptionOverride,
    user
  )
  ...
  if #ingredients.base > 0 and #ingredients.active > config.capacity then
    errors[#errors + 1] = HF.ReplaceString(
      TextManager.Get("lua.chemerror.capacity").Value,
      "{cap}",
...
Hook.Add(
  ...
  function(effect, deltaTime, item, targets, worldPosition)
    ...
    Timer.Wait(function()
      NTP.TryCraftPills(item, user)
    end, 250)
  end
)

Hook.Add(
  "NTP.ChemMaster.makeall",
  "NTP.ChemMaster.makeall",
  function(effect, deltaTime, item, targets, worldPosition)
    -- check if in craftall queue, if so, abort
    for craftall in NTP.ActiveChemCraftalls do

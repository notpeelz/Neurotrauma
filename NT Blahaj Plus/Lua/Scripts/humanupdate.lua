...
function NTBP.PreUpdateHuman(character)
  ...
  if HF.HasTalent(character, "ntbp_blahajpower") then
    ...
    powerPoints = 4 * checkInventoryForItemsRecursive(character.Inventory, "blahajplus")
      + powerPoints
    powerPoints = 10 * checkInventoryForItemsRecursive(character.Inventory, "blahajplusplus")
      + powerPoints
    powerPoints = 10 * checkInventoryForItemsRecursive(character.Inventory, "blahajarmor")
      + powerPoints
    powerPoints = 10 * checkInventoryForItemsRecursive(character.Inventory, "blahajrifle")
      + powerPoints
    powerPoints = 8 * checkInventoryForItemsRecursive(character.Inventory, "sharkmask")
      + powerPoints
    powerPoints = 8 * checkInventoryForItemsRecursive(character.Inventory, "sharkscooter")
      + powerPoints

    if HF.HasTalent(character, "ntbp_freehandouts") then
      -- tally up posters
      powerPoints = 1.5 * checkInventoryForItemsRecursive(character.Inventory, "bpposter_blahaj1")
        + powerPoints
      powerPoints = 1.5 * checkInventoryForItemsRecursive(character.Inventory, "bpposter_blahaj2")
        + powerPoints
      powerPoints = 1.5 * checkInventoryForItemsRecursive(character.Inventory, "bpposter_blahaj3")
        + powerPoints
      powerPoints = 1.5 * checkInventoryForItemsRecursive(character.Inventory, "bpposter_trans")
        + powerPoints
      powerPoints = 2.5
          * checkInventoryForItemsRecursive(character.Inventory, "bpposter_superblahaj")
        + powerPoints
    end

    blahajPower = HF.Clamp(powerPoints, 0, 100)
  end
  HF.ApplyAfflictionChange(
    character,
    "blahajpower",
    blahajPower,
    HF.GetAfflictionStrength(character, "blahajpower"),
    0,
    100
  )
  ...
  if armor ~= nil and armor.Prefab.Identifier.Value == "blahajarmor" then
    ...
    local armorPostersSuperHaj =
      checkInventoryForItemsRecursive(armorInventory, "bpposter_superblahaj")

    blahajArmorProteccPoints = armorHajes * 5
      + armorHajesPlus * 10
      + armorHajesPlusPlus * 25
      + armorPostersHaj1 * 5
      + armorPostersSuperHaj * 5 * 0.8

    blahajArmorSpeedPoints = 2 * armorPostersHaj2 + armorPostersSuperHaj * 2 * 0.8

    blahajArmorRegenPoints = 4 * armorPostersHaj3 + armorPostersSuperHaj * 4 * 0.8

    blahajArmorRegenPoints = 2 * armorPostersTrans + armorPostersSuperHaj * 2 * 0.8
  end
      ...
      if distance < NTBP.PostersRange then
        ...
        if holdableComponent ~= nil and holdableComponent.Attached then
          if inRangePosters[poster.Prefab.Identifier.Value] == nil then
            ...
            inRangePosters[poster.Prefab.Identifier.Value] = inRangePosters[poster.Prefab.Identifier.Value]
              + 1
          end
        end
      end
  ...
  local protecc = (
    (
      (
        ...
        + HF.Clamp(inRangePosters.bpposter_blahaj1 + inRangePosters.bpposter_superblahaj, 0, 3)
          * 15
          * NTBP.PostersPotency
      ) / 200
    ) ^ 0.5
  ...
  local crisis = HF.BoolToNum(HF.HasTalent(character, "ntbp_identitycrisis"), blahajPower)
    + blahajArmorRegenPoints
    + HF.Clamp(inRangePosters.bpposter_trans + inRangePosters.bpposter_superblahaj, 0, 3)
      * 15
      * NTBP.PostersPotency
  local prevspeed = HF.GetAfflictionStrength(character, "blahajspeed")
  local speed = blahajPower / 2
    + blahajArmorSpeedPoints / 2
    + HF.Clamp(inRangePosters.bpposter_blahaj2 + inRangePosters.bpposter_superblahaj, 0, 3)
      * NTBP.PostersPotency
      * 15
  local prevregen = HF.GetAfflictionStrength(character, "blahajregen")
  local regen = blahajArmorRegenPoints / 2
    + HF.Clamp(inRangePosters.bpposter_blahaj3 + inRangePosters.bpposter_superblahaj, 0, 3)
      * NTBP.PostersPotency
      * 10
  ...
  if blahajShield < targetBlahajShield then
    blahajShield = HF.Clamp(blahajShield + NT.Deltatime * 0.1, 0, targetBlahajShield)
  end
...
function NTBP.TickUpdateHuman(character)
  if character.InWater then
    ...
    if
      HF.HasTalent(character, "ntbp_ramventilation")
      or (headwear ~= nil and HF.ItemHasTag(headwear, "ramventilating"))
    then

...
function NTMB.UpdateMetabolism(character)
  if not NTMB.ShouldHaveMetabolism(character) then
    return
  end
  ...
  while energyCost > 0.01 do
    ...
    for key, val in pairs(weightTable) do
      local redeemedEnergy =
        math.min(NTMB.GetNutrientLevel(charID, key, 0), iterationStartEnergyCost * val / weightSum)
  ...
  charDat.stomach.acid = HF.Clamp(
    charDat.stomach.acid
      + 0.5
        * NTMB.Deltatime
        * HF.Clamp((NTMB.GetNutrientLevel(charID, "sodium", 140) / 140) ^ 3, 0, 2),
    0,
    100
  )
  ...
  for contents in charDat.stomach.contents do
    local digestedFraction = math.min(
      contents.fractionLeft,
      charDat.stomach.acid
        / 100
        * 5
        * (contents.eatableData.digestionSpeed or 1)
        * NTConfig.Get("NTMB_digestionSpeed", 1)
        * NTMB.Deltatime
        / #charDat.stomach.contents
        / (contents.eatableData.weight or 1)
    )

    for nutrient, amount in pairs(contents.eatableData.nutrients) do
      if NTMB.Nutrients[nutrient] then
        NTMB.AddNutrientLevel(
          charID,
          nutrient,
          amount * digestedFraction * (NTMB.Nutrients[nutrient].absorptionMultiplier or 1)
        )
      end
    end
  ...
  for key, val in pairs(NTMB.Nutrients) do
    if val.type ~= "energy" then
      if val.normalRange and (val.idleDrainRate or val.exertDrainRate) then
        local prevValue =
          NTMB.GetNutrientLevel(charID, key, (val.normalRange[1] + val.normalRange[2]) / 2)

        -- normalization
        local normalizationFactor = 1
        if prevValue < val.normalRange[1] then
          normalizationFactor = HF.Lerp(
            1,
            prevValue / val.normalRange[1],
            math.min(1, NTConfig.Get("NTMB_rateNormalization", 1))
          )
        ...
        local drainrate = ((val.idleDrainRate or 0) + (val.exertDrainRate or 0) * exertion)
          * normalizationFactor
...
function NTMB.ShouldHaveMetabolism(character)
  ...
  if CLIENT or HF.CharacterToClient(character) then
    return true
  end
  if NTConfig.Get("NTMB_crewAIMetabolsim") and character.TeamID == CharacterTeamType.Team1 then
    return true
  end
  return false
end
...
function NTMB.GetNutrientLevel(charID, nutrient, default)
  ...
  if
    not charDat
    or not charDat.nutrients
    or not charDat.nutrients[nutrient]
    or not charDat.nutrients[nutrient].amount
  then
    return default
  end
...
function NTMB.GetMetabolismData(charID)
  if NTMB.MetabolismData[charID] then
...
function NTMB.ResetMetabolismData(charID)
  local newDat =
    { nutrients = {}, ID = charID, stomach = { acid = 100, hunger = -50, contents = {} } }
    ...
    if not character.IsDead then
      if character.IsHuman then
        ...
        if velocityMagnitude > 0.1 then
          ...
          if
            animController.CurrentAnimationParams == animController.SwimSlowParams
            or animController.CurrentAnimationParams == animController.SwimFastParams
          then
            -- swimming

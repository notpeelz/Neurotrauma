    ...
    func = function(charID, character, level, i)
      ...
      if level < 8 then
        HF.SetAffliction(
          character,
          "immunity",
          HF.Clamp(HF.GetAfflictionStrength(character, "immunity", 100), 1, 100 - (8 - level) * 30)
        )
        NTC.SetSymptomTrue(character, "sym_fever", NTMB.Deltatime / NT.Deltatime + 1)
      end
      -- bleeding from skin
...
Timer.Wait(function()
  NT.Afflictions.sym_thirst = {
    update = function(c, i)
      if c.afflictions.sym_unconsciousness.strength > 0 then
        ...
        local desiredStrength = (55 - waterlevel) * 10 + HF.Clamp((sodiumlevel - 150) * 2, 0, 50)
  ...
  NT.Afflictions.sym_noappetite = {
    update = function(c, i)
      if c.afflictions.sym_unconsciousness.strength > 0 then
        c.afflictions[i].strength = 0
      else
        local sodiumlevel = NTMB.GetNutrientLevel(NTMB.GetCharacterID(c.character), "sodium", 140)
        local vitaminB12level =
          NTMB.GetNutrientLevel(NTMB.GetCharacterID(c.character), "vitaminB12", 400)
        local vitaminDlevel =
          NTMB.GetNutrientLevel(NTMB.GetCharacterID(c.character), "vitaminD", 65)
        local desiredStrength = HF.Clamp((sodiumlevel - 170), 0, 50)

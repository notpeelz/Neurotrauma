Hook.Add(
  ...
  function(effect, deltaTime, item, targets, worldPosition)
    ...
    Timer.Wait(function()
      NTMB.UseCuttingBoard(item, user)
    end, 50)
  end
)

Hook.Add(
  "ntmb.cuttingboard.analyze",
  "ntmb.cuttingboard.analyze",
  function(effect, deltaTime, item, targets, worldPosition)
    local user = nil
...
function NTMB.UseCuttingBoard(board, user)
  ...
  for recipeName, recipe in pairs(NTMB.Recipes) do
    if
      recipe.station == "cuttingboard"
      and (
        (
          ((not recipe.plate) and not plate)
          or (plate and recipe.plate == plate.Prefab.Identifier.Value)
        )
        and (
      ...
      if hasAllNecessaryIngredients then
        if recipe.type == "freeformAssorted" then
          local filteredIngredients =
            NTMB.FilterIngredients(ingredients, recipe.ingredientTag, usedIngredients)
          print(#filteredIngredients)
          for ingredient in filteredIngredients do
            print(ingredient.Prefab.Identifier.Value)
          end
          if
            (#filteredIngredients > 0 or #usedIngredients > 0)
            and #filteredIngredients >= (recipe.minimumFreeformIngredients or 0)
          then
            ...
            for ingredient in filteredIngredients do
              ...
              for nutrient, amount in pairs(eatableData.nutrients) do
                resultEatableData.nutrients[nutrient] = (resultEatableData.nutrients[nutrient] or 0)
                  + amount * fraction * (recipe.nutritionMultiplier or 1)
              end
              resultEatableData.weight = (resultEatableData.weight or 0)
                + (eatableData.weight or 0) * fraction * (recipe.weightMultiplier or 1)
            end
            ...
            for ingredient in usedIngredients do
              ...
              for ingredientRequired in removeNecessaryLeft do
                if
                  ingredientRequired.amount > 0 and availableIdentifier == ingredientRequired.id
                then
                  ...
                  for nutrient, amount in pairs(ingredientEatableData.nutrients) do
                    resultEatableData.nutrients[nutrient] = (
                      resultEatableData.nutrients[nutrient] or 0
                    )
                      + amount * nutrientFraction * (recipe.nutritionMultiplier or 1)
                  end
                  resultEatableData.weight = (resultEatableData.weight or 0)
                    + (ingredientEatableData.weight or 0)
                      * nutrientFraction
                      * (recipe.weightMultiplier or 1)
                end
              end
            end
            ...
            if recipe.descriptionType == "showIngredients" then
              ...
              for ingredient in filteredIngredients do
                local eatableData = NTMB.ItemToEatableData(ingredient)
                local fraction = math.min(eatableData.condition / 100, eatableData.biteSize or 1)
                local identifier = eatableData.sourceIdentifier
                  or ingredient.Prefab.Identifier.Value
                resultEatableData.description = resultEatableData.description
                  .. "\n- "
            ...
            Timer.Wait(function()
              HF.SpawnItemPlusFunction("takeoutbag", function(params1)
                while restFraction > 0 do
                  local outputFraction = math.min(restFraction, desiredFraction)
                    * recipe.desiredYield
                  local slot = 0
                  if outputFraction < 1 then
                    slot = 1
                  end
          ...
          for ingredient in ingredients do
            ...
            for ingredientRequired in removeNecessaryLeft do
              if ingredientRequired.amount > 0 and availableIdentifier == ingredientRequired.id then
                ...
                elseif
                  ingredientEatableData.sourceIdentifier ~= ingredient.Prefab.Identifier.Value
                then
...
function NTMB.EatableDataToTags(data)
  ...
  if data.description ~= nil then
    table.insert(res, "des/" .. data.description)
  end

  -- leftovers
  if data.leftovers ~= nil then
    table.insert(res, "ntmbl/" .. data.leftovers)
  end
  ...
  if data.color ~= nil then
    table.insert(
      ...
      "col/"
        ...
        .. "/"
        .. tostring(HF.Round(data.color[3]))
    )
  end
...
function NTMB.AnalyzeDish(dish, user)
  ...
  local readoutstring = "Dish analysis of "
    .. HF.GetText("entityname." .. dish.Prefab.Identifier.Value)
    .. ":\n"

  readoutstring = readoutstring
    .. "weight: "
    .. tostring(HF.Round(eatableData.weight * fraction))
    .. "g\n"
    ...
    for nutrient, amount in pairs(eatableData.nutrients) do
      ...
      if not val.hiddenOnReadout then
        ...
        nutrientstring = nutrientstring
          .. "\n"
          .. HF.GetText("ntmb.nutrients." .. nutrient)
          .. ": "
          .. strength
          .. val.foodUnit

...
function NTMB.FilterIngredients(ingredients, tag, excludeItems)
  ...
  for item in ingredients do
    ...
    if
      (not HF.TableContains(excludeItems, item))
      and (
        HF.ItemHasTag(item, tag)
        or (
...
function NTMB.UpdateMachines()
  ...
  for stove in NTMB.Stoves do
    if stove.Removed then
      table.insert(toRemove, stove)
    end
  end
  for item in toRemove do
    NTMB.Stoves[item] = nil
  end

  toRemove = {}
  for oven in NTMB.Ovens do
    if oven.Removed then
      table.insert(toRemove, oven)
    end
  end
  for item in toRemove do
    NTMB.Ovens[item] = nil
  ...
  for fryer in NTMB.Fryers do
    if fryer.Removed then
      table.insert(toRemove, fryer)
    end
  end
  for item in toRemove do
    NTMB.Fryers[item] = nil
  end
...
function NTMB.UpdateHeaterMachine(machine, type)
  for item in machine.OwnInventory.AllItems do
    ...
    for recipeName, recipe in pairs(NTMB.Recipes) do
      if
        recipe.station == type
        and HF.TableContains(recipe.from or {}, item.Prefab.Identifier.Value)
      then
        local success = false
        ...
        if hasAllNecessaryIngredients then
          ...
          if NTMB.ItemData[item].temperature >= (recipe.temperature or 1) then
            if recipe.type == "turnInto+keepNutrition" then
              ...
              resultEatableData =
                NTMB.MultiplyNutrition(resultEatableData, recipe.nutritionMultiplier or 1)
            ...
            elseif recipe.type == "turnInto" then
              ...
              Timer.Wait(function()
                ...
                HF.SpawnItemPlusFunction(recipe.result, function(params)
                  params.item.Condition = resultCondition
                end, {}, machine.OwnInventory, resultSlot)
              end, 50)
              Timer.Wait(function()
                HF.RemoveItem(item)
              end, 1)
            ...
            elseif recipe.type == "potFluid" then
              Timer.Wait(function()
                HF.ReplaceItemIdentifier(item, recipe.result, true)
              end, 1)
              success = true
            end
            ...
            if success and recipe.ingredients then
              ...
              for ingredient in ingredients do
                for ingredientRequired in removeNecessaryLeft do
                  local ingredientEatableData = NTMB.ItemToEatableData(ingredient)
                  if
                    ingredientRequired.amount > 0
                    and ingredientEatableData.sourceIdentifier == ingredientRequired.id
                  then
                    local fractionRemoved =
                      math.min(ingredientEatableData.condition / 100, ingredientRequired.amount)
                    ...
                    then
                      HF.ReplaceItemIdentifier(
                        ingredient,
                        ingredientEatableData.sourceIdentifier,
                        true
                      )
    ...
    if
      type == "fryer"
      and (not HF.ItemHasTag(item, "finalFryerResult"))
      and not fittingRecipeFound
      and NTMB.ItemData[item].temperature >= 10
    then
      local resultSlot = machine.OwnInventory.FindIndex(item)
      Timer.Wait(function()
        -- spawn in the result

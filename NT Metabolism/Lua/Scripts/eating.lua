...
Hook.Add(
  "item.applyTreatment",
  "NTMB.healthinterfaceeating",
  function(item, usingCharacter, targetCharacter, limb)
    ...
    -- force feeding
    if (not NTConfig.Get("NTMB_forceFeeding", true)) and usingCharacter ~= targetCharacter then
      return
    end
    ...
    NTMB.EatItem(
      item,
      eatableData,
      NTMB.GetCharacterID(targetCharacter),
      eatableData.biteSize,
      eatableData.biteSize
    )
  end
)
...
Hook.Add("item.secondaryUse", "NTMB.manualeating", function(item, character)
  ...
  if eatableData then
    if eatableData.disableEating then
      return
    end
...
function NTMB.EatItem(item, eatableData, charID, fraction, conditionloss)
  if item.Removed or not eatableData then
    return
  end
  ...
  for contents in metabolismData.stomach.contents do
    ...
    if matches then
      for nutrient, amount in pairs(consumedContents.eatableData.nutrients) do
        if
          not contents.eatableData.nutrients[nutrient]
          or contents.eatableData.nutrients[nutrient] ~= amount
        then
          matches = false
          break
        end
...
function NTMB.RefreshStuffedness(charID, updateHunger)
  ...
  if updateHunger then
    local hungerChange = NTMB.Deltatime
      * (0.1 - weightInStomach / 500)
      * NTConfig.Get("NTMB_hungerSpeed", 1) -- 50g in the stomach is enough to counteract hunger gain
    if hungerChange < 0 then
      hungerChange = hungerChange * 4
    end -- if theres enough in your stomach, hunger recovery is quicker
...
function NTMB.Vomit(character)
  ...
  for contents in charDat.stomach.contents do
    local lostFraction =
      math.min(contents.fractionLeft, 400 / #charDat.stomach.contents / contents.eatableData.weight)

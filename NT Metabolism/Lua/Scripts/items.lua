        ...
        if val.normalRange then
          if strength < val.normalRange[1] then
            issuestring = issuestring
              .. "\n! low "
              .. HF.GetText("ntmb.nutrients." .. key)
              .. " "
              .. tostring(strength)
              .. " << "
              .. tostring(val.normalRange[1] .. "-" .. tostring(val.normalRange[2]))
              .. " ("
              .. tostring(HF.Round(strength / val.normalRange[1] * 100))
              .. "% of lower normal)"
          elseif strength > val.normalRange[2] then
            issuestring = issuestring
              ...
              .. tostring(
                val.normalRange[1]
                  .. "-"
                  .. tostring(val.normalRange[2])
                  .. " >> "
                  .. tostring(strength)
              )
              .. " ("
              .. tostring(HF.Round(strength / val.normalRange[2] * 100))
              .. "% of upper normal)"
          end
        end
...
Hook.Add(
  ...
  function(effect, deltaTime, item, targets, worldPosition)
    ...
    if item.InWater then
      ...
      if not NTConfig.Get("NTMB_taintedWater", true) then
        spawnIdentifier = "pot_water"
      end
  ...
  Humans = {
    ...
    finalFunction = function(character, butcherer, skill)
      local organData = {
        {
          dmgAff = "liverdamage",
          fooddrop = "livertransplant_food",
          transplant = "livertransplant_q1",
        },
        {
          dmgAff = "lungdamage",
          fooddrop = "lungtransplant_food",
          transplant = "lungtransplant_q1",
        },
        {
          dmgAff = "kidneydamage",
          fooddrop = "kidneytransplant_food",
          transplant = "kidneytransplant_q1",
        },
        {
          dmgAff = "heartdamage",
          fooddrop = "hearttransplant_food",
          transplant = "hearttransplant_q1",
        },
        {
          dmgAff = "cerebralhypoxia",
          fooddrop = "braintransplant_food",
          transplant = "braintransplant_q1",
        },
      }
      ...
      for data in organData do
        ...
        if HF.Chance(dropChance) then
          ...
          local transplantChance = HF.Minimum((100 - dmg) / 100, 0.2, 0) -- organ condition
          ...
          HF.SpawnItemPlusFunction(dropID, function(params)
            params.item.Condition = dropCondition
          end, nil, nil, nil, spawnPos)
        end
      end
...
function NTMB.GetCookingSkill(character)
  return HF.GetSkillLevel(character, "cooking")
end
Hook.Add(
  "ntmb.butcher.progress",
  "ntmb.butcher.progress",
  function(effect, deltaTime, item, targets, worldPosition)
    local user = item.ParentInventory.Owner
    local target = targets[1]
    if not target or not user or target == user then
      return
    end

    if not NTMB.CorpseButcherProgress[target] then
      NTMB.CorpseButcherProgress[target] = 0
    end
    ...
    for data in NTMB.ButcheringData do
      if HF.TableContains(data.species, target.SpeciesName.Value) then
        ...
        NTMB.CorpseButcherProgress[target] = NTMB.CorpseButcherProgress[target]
          + dropRolls / data.size * 10
        ...
        if data.processDrops then
          while dropRolls > 0 do
            ...
            for drop in data.processDrops do
              if
                HF.Chance(chanceMult * ((drop.chance or 1) + skill * (drop.chancePerSkill or 0)))
              then

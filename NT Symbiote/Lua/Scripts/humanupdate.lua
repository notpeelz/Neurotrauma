...
Timer.Wait(function()
    ...
    update = function(c, i)
      if c.afflictions[i].strength > 0.1 then
        ...
        if c.afflictions.lungdamage.strength < 99 then
          c.afflictions.lungdamage.strength = c.afflictions.lungdamage.strength
            - 0.04 * NT.Deltatime
        end
  ...
  NT.Afflictions.surgery_huskhealth = {
    ...
    update = function(c, i)
      ...
      if not NTS.HF.HasHusk(c.character) then
        c.afflictions[i].strength = 0
        return
      end
  ...
  NT.Afflictions.bonecuttorso = {
    ...
    update = function(c, i)
      ...
      if
        ...
        and NTS.HF.HasHusk(c.character)
      then
  ...
  NT.Afflictions.huskstasis = {
    ...
    update = function(c, i)
      ...
      c.afflictions.traumaticshock.strength = c.afflictions.traumaticshock.strength
        - 5 * NT.Deltatime
    ...
    if HF.GetInnerWearIdentifier(character) == "zealotrobes" and NTS.HF.HasHusk(character) then
      ...
      for limbType in limbTypes do
        if HF.GetAfflictionStrengthLimb(character, limbType, "gangrene") < 15 then
          HF.AddAfflictionLimb(character, "gangrene", limbType, -0.1 * NT.Deltatime)
        end
      end
    end
  end

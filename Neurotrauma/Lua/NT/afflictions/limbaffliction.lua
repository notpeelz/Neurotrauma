...
function M.Apply(o, c)
  ...
  c.ca:set(o.def, o.limbType, o.newValue)
end
...
local function afUpdateState(self, c)
  local value = c.ca:get(self.def, self.limbType)
...
local function createState(def, limbType)
  local o = {
    limbType = limbType,
    value = 0,
    newValue = 0,
...
function M.Create(e)
  ...
  if type(e.targetLimbs) ~= "table" then
    error("invalid or missing required field: targetLimbs", 2)
  end
  ...
  -- TODO: check if it's a limb affliction (can we check if the target limbs are valid?)

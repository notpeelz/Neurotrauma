...
Hook.Add(
  ...
  function(effect, deltaTime, item, targets, worldPosition)
    ...
    if target.IsDead then
      ...
      item.SendSignal(
        tostring(HF.Round(HF.GetAfflictionStrength(target, "bloodpressure", 100))),
        "bloodpressure_out"
      )
    end
    item.SendSignal(
      tostring(HF.Round(100 - HF.GetAfflictionStrength(target, "hypoxemia", 0))),
      "bloodoxygen_out"
    )
    item.SendSignal(
      tostring(HF.Round(HF.GetAfflictionStrength(target, "cerebralhypoxia", 0))),
      "neurotrauma_out"
    )
    item.SendSignal(
      tostring(HF.Round(HF.GetAfflictionStrength(target, "organdamage", 0))),
      "organdamage_out"
    )
    ...
    item.SendSignal(
      tostring(HF.BoolToNum(HF.HasAffliction(target, "surgeryincision"), 1)),
      "insurgery_out"
    )

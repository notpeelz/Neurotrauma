...
NT.Afflictions = {
  ...
  immunity = {
    ...
    update = function(c, i)
      ...
      c.afflictions[i].strength = HF.Clamp(
        c.afflictions[i].strength + (0.5 + c.afflictions[i].strength / 100) * NT.Deltatime,
        5,
        100
      )
    end,
    ...
    update = function(c, i)
      ...
      local desiredbloodpressure = (
        ...
        * HF.Clamp(
          ...
          0,
          1
        ) -- none if half or more traumatic shock
  ...
  hypoxemia = {
    update = function(c, i)
      ...
      c.stats.availableoxygen =
        math.min(c.stats.availableoxygen, 100 - c.afflictions.pneumothorax.strength / 2)
  ...
  internalbleeding = {
    update = function(c, i)
      ...
      c.afflictions[i].strength = c.afflictions[i].strength
        - NT.Deltatime * 0.02 * c.stats.clottingrate
      if c.afflictions[i].strength > 0 then
        ...
        + (
            ...
            + HF.Clamp(
              (
                ...
                * HF.BoolToNum(
                  c.afflictions.alv.strength <= 0.1
                    and not HF.HasAffliction(c.character, "cpr_buff")
                )
    ...
    update = function(c, i)
      if not c.stats.stasis then
        c.afflictions[i].strength = c.afflictions[i].strength
          ...
          + HF.Clamp(HF.GetAfflictionStrength(c.character, "nausea", 0), 0, 1)
            * 0.1
            * NT.Deltatime
      end
  ...
  stroke = {
    update = function(c, i)
      ...
      c.afflictions[i].strength = c.afflictions[i].strength
        - (1 / 20) * c.stats.clottingrate * NT.Deltatime
  ...
  givein = {
    ...
    update = function(c, i)
      c.afflictions[i].strength = HF.BoolToNum(
        c.afflictions.t_paralysis.strength > 0 or c.afflictions.sym_unconsciousness.strength > 0
      )
    end,
  },
  lockedhands = {
    update = function(c, i)
      ...
      c.afflictions[i].strength =
        HF.BoolToNum((c.stats.lockleftarm and c.stats.lockrightarm) or handcuffed, 100)
    end,
  },
  traumaticshock = {
    update = function(c, i)
  ...
  tachycardia = {
    update = function(c, i)
      ...
      c.afflictions[i].strength =
        math.max(c.afflictions[i].strength, HF.BoolToNum(hasSymHarmless, 2))
  ...
  hypoventilation = {
    update = function(c, i)
      ...
      if
        c.afflictions.hyperventilation.strength > 0 and c.afflictions.hypoventilation.strength > 0
      then
...
NT.LimbAfflictions = {
  bandaged = {
    update = function(c, limbaff, i)
      ...
      local bandageDirtifySpeed = 0.1
        + HF.Clamp(wounddamage / 100, 0, 0.4)
        + limbaff.bleeding.strength / 20
      ...
      if limbaff.dirtybandage.strength > 0 then
        limbaff.dirtybandage.strength = limbaff.dirtybandage.strength
          + bandageDirtifySpeed * NT.Deltatime
      end
  ...
  infectedwound = {
    update = function(c, limbaff, i)
      ...
      if limbaff.dirtybandage.strength > 10 and wounddamage > 5 then
        infectindex = infectindex
          + (wounddamage / 40 + limbaff.dirtybandage.strength / 20) * NT.Deltatime
      end
    ...
    update = function(c, limbaff, i)
      limbaff[i].strength = limbaff[i].strength
        + (
            -0.1
            + HF.BoolToNum(
              limbaff.infectedwound.strength > 10 or limbaff.foreignbody.strength > 15,
              0.15
            )
          )
          * NT.Deltatime
  ...
  sedated = {
    getter = function(c)
      return c.afflictions.analgesia.strength > 0
        ...
        or c.afflictions.afadrenaline.strength > 0

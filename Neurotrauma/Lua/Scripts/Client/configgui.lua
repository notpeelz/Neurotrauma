  ...
  for key, entry in pairs(NTConfig.Entries) do
    if entry.difficultyCharacteristics then
      ...
      difficulty = difficulty
        + math.min(entryValue * diffMultiplier, entry.difficultyCharacteristics.max or 1)
    end
  end
...
NT.ShowGUI = function()
  local frame = GUI.Frame(
    GUI.RectTransform(Vector2(0.3, 0.6), GUI.Screen.Selected.Frame.RectTransform, GUI.Anchor.Center)
  )
  NT.GUIFrame = frame
  frame.CanBeFocused = true
  local config =
    GUI.ListBox(GUI.RectTransform(Vector2(1, 1), frame.RectTransform, GUI.Anchor.BottomCenter))

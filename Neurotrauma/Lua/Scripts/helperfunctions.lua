...
function NT.LimbIsAmputated(character, limbtype)
  return NT.LimbIsTraumaticallyAmputated(character, limbtype)
    or NT.LimbIsSurgicallyAmputated(character, limbtype)
end
...
function HF.HasAfflictionExtremity(character, identifier, limbtype, minamount)
  ...
  if
    ...
    or limbtype == LimbType.LeftForearm
    or limbtype == LimbType.LeftHand
  then
    aff = character.CharacterHealth.GetAffliction(
      identifier,
      character.AnimController.GetLimb(LimbType.LeftArm)
    )
    if aff == nil then
      aff = character.CharacterHealth.GetAffliction(
        identifier,
  ...
  elseif
    ...
    or limbtype == LimbType.RightForearm
    or limbtype == LimbType.RightHand
  then
    aff = character.CharacterHealth.GetAffliction(
      identifier,
      character.AnimController.GetLimb(LimbType.RightArm)
    )
    if aff == nil then
      aff = character.CharacterHealth.GetAffliction(
        identifier,
  ...
  elseif
    ...
    or limbtype == LimbType.LeftThigh
    or limbtype == LimbType.LeftFoot
  then
    aff = character.CharacterHealth.GetAffliction(
      identifier,
      character.AnimController.GetLimb(LimbType.LeftLeg)
    )
    if aff == nil then
      aff = character.CharacterHealth.GetAffliction(
        identifier,
    ...
    if aff == nil then
      aff = character.CharacterHealth.GetAffliction(
        identifier,
        character.AnimController.GetLimb(LimbType.LeftFoot)
      )
    end
  elseif
    limbtype == LimbType.RightLeg
    or limbtype == LimbType.RightThigh
    or limbtype == LimbType.RightFoot
  then
    aff = character.CharacterHealth.GetAffliction(
      identifier,
      character.AnimController.GetLimb(LimbType.RightLeg)
    )
    if aff == nil then
      aff = character.CharacterHealth.GetAffliction(
        identifier,
...
function HF.ApplyAfflictionChange(
  character,
  identifier,
  strength,
  prevstrength,
...
function HF.ApplyAfflictionChangeLimb(
  character,
  limbtype,
  identifier,
  strength,
...
function HF.GiveSkillScaled(character, skilltype, amount)
  if character ~= nil and character.Info ~= nil then
    HF.GiveSkill(
      character,
      skilltype,
      amount * 0.001 / math.max(HF.GetSkillLevel(character, skilltype), 1)
    )
  end
end
...
function HF.ApplyEndocrineBoost(character, talentlist)
  ...
  for talent in talentlist do
    if
      not HF.TableContains(disallowedTalents, talent)
      and not HF.TableContains(characterTalents, talent)
    then
...
function HF.ReplaceItemIdentifier(item, newIdentifier, keepCondition)
  ...
  if item.OwnInventory ~= nil then
    for containedItem in item.OwnInventory.AllItems do
      table.insert(
        containedItems,
        { item = containedItem, slot = item.OwnInventory.FindIndex(containedItem) }
      )
    end
  end
  ...
  Timer.Wait(function()
    HF.SpawnItemPlusFunction(newIdentifier, function(params)
      for containedItem in params.containedItems do
        params.item.OwnInventory.TryPutItem(
          containedItem.item,
          containedItem.slot,
          true,
          false,

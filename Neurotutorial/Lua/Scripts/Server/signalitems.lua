...
local function SpawnSubject(position, name, job, team, dontgivejobitems, dontgiveobjective)
  ...
  local character =
    Character.Create(info, position, tostring(math.random(0, 1000000)), 0, false, true)
  ...
  if not dontgiveobjective then
    ...
    local order = Order(orderPrefab, OrderTarget(position, hull)).WithManualPriority(
    ...
    startfunc = function(item, data)
      Timer.Wait(function()
        ...
        Timer.Wait(function()
          for i = 1, 4, 1 do
            local spawnPos =
              Vector2(item.WorldPosition.X + (((i % 2) - 0.5) * 2 * 0), item.WorldPosition.Y)
            local raptor =
              Character.Create("mudraptor", spawnPos, tostring(math.random(0, 1000000)))
            table.insert(data.raptors, raptor)
            character.AIController.AddCombatObjective(
...
NTTut.signalReceivedMethods.nttut_subjectcomponent = function(signal, connection)
  ...
  if connection.Name == "in_scenario" then
    local memComponents =
      NTTut.HF.EnumerableToTable(connection.Item.GetComponents(Components.MemoryComponent))
